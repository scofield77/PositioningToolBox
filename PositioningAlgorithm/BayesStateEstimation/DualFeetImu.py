...
class DualFeetImu:
    ...
    def state_transaction_function(self, l_imu_data,
        ...
        self.state[self.r_offset + 3:self.r_offset + 6] = self.state[
                                                          self.r_offset + 3:self.r_offset + 6] + r_acc * self.time_interval
    ...
    def measurement_zv(self, left_zv_flag, right_zv_flag, max_distance=2.5):
        ...
        if left_zv_flag > 0.5 and right_zv_flag > 0.5:
            ...
            cov_matrix = np.identity(6) * 0.001

        elif left_zv_flag > 0.5:
            H = np.zeros([3, 18])
            H[0:3, 3:6] = np.identity(3)

            m = np.zeros(3)
            cov_matrix = np.identity(3) * 0.001

        elif right_zv_flag > 0.5:
            H = np.zeros([3, 18])
        ...
        print(self.state[3:6], self.state[12:15])
